# Sign Bridge Model Release: sign_bridge_v2

## Lineage
- Source Experiment: `exp_mock`
- Recommendation passed via `final_eval.py` test-set verification.

## Preprocessing Specification
To ensure compatibility with this model, the frontend MUST perform the following steps identically:
1. **Landmark Extraction**: Extract exactly 21 hand landmarks per frame.
2. **Wrist Reference**: Landmark index 0 (Wrist) is the origin.
3. **Normalization Formula**: For each point `i`:
   `tx_i = x_i - x_0`
   `ty_i = y_i - y_0`
   `tz_i = z_i - z_0`
4. **Scaling**: Compute scale factor `S = distance(wrist, middle_finger_mcp)`. 
   `S = sqrt(tx_9^2 + ty_9^2 + tz_9^2)`
   Divide every translated coordinate by `S`.
5. **Flattening Order**: For each landmark 0 through 20 sequentially, append `[nx, ny, nz]`, resulting in a flat 63-element vector per frame.
...
## Parity Test Report
PENDING (Run parity check to append results)
